...
import {
  Eye, EyeOff, Mail, Lock, User, Layers, ArrowRight, Check,
} from 'lucide-react'
import { useAuth } from '@/context/AuthContext'

/* ── Wallpaper collage — different set from sign-in ── */
const COLLAGE = [
  { id: '1462275646964-a0e3386b89ae', tall: false },
  { id: '1538370965046-79c0d0568add', tall: true  },
  { id: '1518895949257-6b925b96a492', tall: false },
  { id: '1491895200222-0fc4a4c35e18', tall: true  },
  { id: '1501854140801-50d01698950b', tall: false },
  { id: '1446776877081-d282a0f896e2', tall: true  },
]
...
function calcStrength(pw) {
  ...
  return              { score: 5, label: 'Great',  color: 'bg-emerald-400' }
}
...
export default function SignUpPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      /* orbs */
      gsap.to(orb1Ref.current, { x: -65, y: 55,  duration: 9,  ease: 'sine.inOut', repeat: -1, yoyo: true })
      gsap.to(orb2Ref.current, { x: 75,  y: -45, duration: 11, ease: 'sine.inOut', repeat: -1, yoyo: true, delay: 1.5 })
      gsap.to(orb3Ref.current, { x: -40, y: -35, duration: 7,  ease: 'sine.inOut', repeat: -1, yoyo: true, delay: 3 })

      /* particles */
      partRefs.current.forEach((el, i) => {
        if (!el) return
        gsap.to(el, {
          y: -28 - (i % 6) * 10, opacity: 0,
      ...
      gsap.fromTo(imgRefs.current,
        { opacity: 0, y: 55, scale: 0.87, rotateX: 10 },
        { opacity: 1, y: 0, scale: 1, rotateX: 0, duration: 0.9, stagger: 0.1, ease: 'power3.out', delay: 0.3 }
      )

      /* brand text */
      gsap.fromTo('.su-brand > *',
        { opacity: 0, x: -30 },
        { opacity: 1, x: 0, duration: 0.65, stagger: 0.1, ease: 'power3.out', delay: 0.2 }
      )
      ...
      /* form fields */
      gsap.fromTo('.su-field',
        { opacity: 0, y: 20 },
        { opacity: 1, y: 0, duration: 0.5, stagger: 0.07, ease: 'power3.out', delay: 0.42 }
      )
    }, wrapRef)
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim() || !email.trim() || !password || !confirm) {
      ...
      gsap.fromTo('.su-error', { x: -8 }, { x: 0, duration: 0.5, ease: 'elastic.out(1, 0.35)' })
      return
    }
    if (!/\S+@\S+\.\S+/.test(email)) {
  ...
  return (
    <div ref={wrapRef} className="min-h-screen flex bg-[#080808] overflow-hidden">
      ...
      >
        {/* Orbs */}
        <div ref={orb1Ref} className="absolute top-[15%] right-[15%] w-96 h-96 rounded-full bg-indigo-600/18 blur-[110px] pointer-events-none" />
        <div ref={orb2Ref} className="absolute bottom-[20%] left-[10%] w-80 h-80 rounded-full bg-violet-500/18 blur-[90px]  pointer-events-none" />
        <div ref={orb3Ref} className="absolute top-[50%] right-[40%] w-56 h-56 rounded-full bg-pink-600/12  blur-[80px]  pointer-events-none" />
        ...
        <div className="absolute inset-0 bg-gradient-to-l from-[#080810]/70 to-transparent pointer-events-none" />

        {/* Brand + perks */}
        <div className="su-brand absolute bottom-10 left-12 max-w-[300px] z-10">
          ...
          <h2 className="text-[1.85rem] font-black text-white mb-3 leading-tight tracking-tight">
            Your next favourite<br />
            <span className="gradient-text">wallpaper awaits</span>
          </h2>
          <p className="text-sm text-zinc-400 leading-relaxed mb-6">
            Create a free account and unlock the full experience.
          </p>

          <ul className="space-y-2.5">
            {PERKS.map((perk, i) => (
              <li key={i} className="flex items-center gap-2.5 text-sm text-zinc-300">
                <span className="w-4.5 h-4.5 rounded-full bg-violet-500/20 border border-violet-500/30 flex items-center justify-center shrink-0">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-10 relative overflow-y-auto">
        ...
        <div ref={formRef} className="w-full max-w-[360px] relative my-auto">
          ...
          {success ? (
            ...
            <>
              <h1 className="su-field text-3xl font-black text-white mb-1 tracking-tight">Create account</h1>
              <p  className="su-field text-sm text-zinc-500 mb-7">Get started free — no credit card needed</p>

              {/* Social */}
              <div className="su-field flex gap-3 mb-7">
                <SocialBtn icon={<GoogleIcon />} label="Google" />
                <SocialBtn icon={<GithubIcon />} label="GitHub" />
              </div>

              {/* Divider */}
              <div className="su-field flex items-center gap-3 mb-7">
                <div className="flex-1 h-px bg-white/[0.07]" />
                <span className="text-[10px] text-zinc-600 uppercase tracking-[0.18em]">or sign up with email</span>
                <div className="flex-1 h-px bg-white/[0.07]" />
              </div>

              {/* Error */}
              {error && (
                <div className="su-error mb-5 px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm flex items-center gap-2">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="su-field">
                  <label className="block text-[11px] font-semibold text-zinc-400 mb-1.5 uppercase tracking-[0.12em]">Full name</label>
                  <div className="relative group">
                    <User size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-violet-400 transition-colors duration-200" />
                    <input
                      type="text"
                      value={name}
                ...
                {/* Email */}
                <div className="su-field">
                  <label className="block text-[11px] font-semibold text-zinc-400 mb-1.5 uppercase tracking-[0.12em]">Email address</label>
                  <div className="relative group">
                ...
                {/* Password */}
                <div className="su-field">
                  <label className="block text-[11px] font-semibold text-zinc-400 mb-1.5 uppercase tracking-[0.12em]">Password</label>
                  <div className="relative group">
                    ...
                    <div className="mt-2 space-y-1">
                      <div className="flex gap-1">
                        {[1,2,3,4,5].map(n => (
                          <div
                            key={n}
                            className={`flex-1 h-1 rounded-full transition-all duration-300 ${n <= strength.score ? strength.color : 'bg-white/10'}`}
                          />
                        ))}
                      </div>
                      <p className="text-[10px] text-zinc-500">
                        Strength: <span className="text-zinc-300 font-medium">{strength.label}</span>
                      </p>
                    </div>
                  )}
                </div>
                ...
                {/* Confirm password */}
                <div className="su-field">
                  <label className="block text-[11px] font-semibold text-zinc-400 mb-1.5 uppercase tracking-[0.12em]">Confirm password</label>
                  <div className="relative group">
                    <Lock size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-violet-400 transition-colors duration-200" />
                    <input
                      ...
                      autoComplete="new-password"
                      className={`w-full pl-10 pr-11 py-3 bg-white/[0.04] border rounded-xl text-sm text-white placeholder-zinc-600 focus:outline-none focus:ring-2 transition-all duration-200 ${
                        confirm.length > 0
                          ? pwMatch
                            ? 'border-green-500/50 focus:border-green-500/70 focus:ring-green-500/15'
                            : 'border-red-500/40  focus:border-red-500/60  focus:ring-red-500/15'
                          : 'border-white/[0.08] focus:border-violet-500/60 focus:ring-violet-500/15 focus:bg-white/[0.06]'
                      }`}
                    />
                    <button
                      type="button"
                    ...
                    {confirm.length > 0 && (
                      <div className={`absolute right-9 top-1/2 -translate-y-1/2 transition-all duration-200 ${pwMatch ? 'text-green-400' : 'text-red-400'}`}>
                        {pwMatch
                ...
                {/* Terms */}
                <div className="su-field flex items-start gap-3 py-0.5">
                  <button
                    type="button"
                ...
                <div className="su-field pt-2">
                  ...
                  >
                    ...
                    {loading ? (
                      ...
                      <>Create Account <ArrowRight size={15} className="group-hover:translate-x-0.5 transition-transform duration-200" /></>
                    )}
                  </button>
                </div>
              </form>

              <p className="su-field mt-7 text-center text-sm text-zinc-600">
                Already have an account?{' '}
                <Link href="/signin" className="text-violet-400 hover:text-violet-300 font-semibold transition-colors">
                  Sign in
                </Link>
              </p>
            </>
  ...
  return (
    <div ref={ref} className="text-center py-8">
      ...
      <div className="relative w-20 h-20 mx-auto mb-6">
        <div className="w-20 h-20 rounded-full bg-gradient-to-br from-violet-500/20 to-indigo-500/20 border border-violet-500/30 flex items-center justify-center">
          <Check size={32} className="text-violet-400" strokeWidth={2.5} />
        </div>
      ...
      <p className="text-sm text-zinc-500 mb-1">Your account has been created.</p>
      <p className="text-xs text-zinc-600">Redirecting you to the homepage…</p>
    </div>
  )
}
